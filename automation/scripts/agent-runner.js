...
function normalizeAgentAnalysis(response, selectedFiles, goalMode) {
  ...
  // Build a lookup that resolves model-emitted paths (repo:X, api/X, X, wrong-prefix:X) to actual selected paths
  const bareToSelected = new Map();
  for (const sp of selectedPaths) {
    const spSuffix = sp.includes(":") ? sp.split(":").slice(1).join(":") : sp;
    bareToSelected.set(spSuffix, sp);
  }

  function resolveEditPath(rawPath) {
    if (selectedPaths.has(rawPath)) return rawPath;
    // Strip colon-based prefix: "repo:functions/..." → "functions/..."
    const colonStripped = rawPath.includes(":") ? rawPath.split(":").slice(1).join(":") : rawPath;
    if (bareToSelected.has(colonStripped)) return bareToSelected.get(colonStripped);
    // Strip slash-based prefix: "api/functions/..." → "functions/..."
    const slashStripped = rawPath.replace(/^[a-zA-Z0-9_-]+\//, "");
    if (bareToSelected.has(slashStripped)) return bareToSelected.get(slashStripped);
    return rawPath; // return as-is, will fail selectedPaths.has check downstream
  }
  ...
  const findings = Array.isArray(response.findings)
    ? response.findings
        .map((finding) => ({
          ...
          file_paths: Array.isArray(finding.file_paths) ? finding.file_paths.map(resolveEditPath).filter((item) => selectedPaths.has(item)) : [],
  ...
  if (goalMode === "audit") {
    ...
    base.safe_edits = Array.isArray(response.safe_edits)
      ? response.safe_edits
          .map((edit) => {
            const kind = String(edit.kind || "patch").trim();
            const editPath = resolveEditPath(String(edit.path || "").trim());
            const summary = String(edit.summary || "").trim();
            const confidence = Number(edit.confidence || 0);

            if (kind === "patch") {
              const search = typeof edit.search === "string" ? edit.search : "";
              const replace = typeof edit.replace === "string" ? edit.replace : "";
              if (!search.trim()) return null;
              return { path: editPath, kind: "patch", summary, confidence, search, replace };
            }
            // Legacy full-file rewrite support
            const content = typeof edit.content === "string" ? edit.content : "";
            if (!content.trim()) return null;
            return { path: editPath, kind: "rewrite", summary, confidence, content };
          })
          .filter((edit) => edit && selectedPaths.has(edit.path))
          .slice(0, 4)
      : [];
  }
