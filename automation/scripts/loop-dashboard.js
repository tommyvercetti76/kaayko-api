...
function buildDashboardHtml(summary) {
  ...
  const recentRows = summary.recent_runs.slice(0, 15).map((run, idx) => {
    ...
    const followupsHtml = followups.length
      ...
      : "";
    // Show inspected files INLINE in the table
    const fileListShort = inspected.slice(0, 3).map((f) => {
      const p = typeof f === "string" ? f : f.path || f.relative_path || String(f);
      return p.split("/").pop();
    }).join(", ") + (inspected.length > 3 ? ` +${inspected.length - 3}` : "");
    return `<tr class="run-row" onclick="toggleDetail('${detailId}')">
      <td>${statusIcon(run.status)}</td>
      <td><span class="status-badge s-${esc(run.status.replace(/_/g, "-"))}">${esc(run.status)}</span>${run.agent_goal_mode === "audit" ? ' <span class="chip">AUDIT</span>' : ' <span class="chip" style="color:var(--green)">EDIT</span>'}</td>
      <td>${esc(run.track)}</td>
      <td class="mono" style="font-size:.72rem;max-width:240px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${esc(run.run_id)}">${esc(run.run_id.slice(0, 40))}</td>
      <td class="mono" style="font-size:.72rem;color:var(--text-dim)">${fileListShort || "\u2014"}</td>
    ...
    <tr class="detail-row" id="${detailId}" style="display:none">
      <td colspan="6">
        <div class="detail-panel">
          ...
          ${applied.length ? `<div class="detail-section"><b>\u270f\ufe0f Files Edited (${applied.length}):</b><div class="detail-files">${applied.map((f) => `<code class="applied-file">${esc(typeof f === "string" ? f : f.path || String(f))}</code>`).join(" ")}</div>${(run.agent_applied_edits_detail || []).map((e) => `<div class="detail-edit"><code>${esc(e.path)}</code> <span class="detail-text">${esc(e.summary || "")}</span></div>`).join("")}</div>` : ""}
          ${inspected.length ? `<div class="detail-section"><b>\ud83d\udd0d Files Inspected (${inspected.length}):</b><div class="detail-files">${inspected.map((f) => `<code>${esc(typeof f === "string" ? f : f.path || f.relative_path || String(f))}</code>`).join(" ")}</div></div>` : ""}
          ${run.agent_verification ? `<div class="detail-section"><b>Verification:</b> syntax=${run.agent_verification.syntax_passed ? "\u2713" : "\u2717"} smoke=${run.agent_verification.smoke_test_skipped ? "\u2298" : run.agent_verification.smoke_test_passed ? "\u2713" : "\u26a0"} \u2192 <span class="status-badge s-${run.agent_verification.summary === "all_passed" ? "approved" : "changes-requested"}">${esc(run.agent_verification.summary)}</span></div>` : ""}
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .nav-link{font-family:var(--mono);font-size:.82rem;font-weight:600;color:var(--purple);background:rgba(167,139,250,0.1);border:1px solid rgba(167,139,250,0.3);border-radius:8px;padding:8px 16px;text-decoration:none;display:inline-flex;align-items:center;gap:8px;transition:all .2s}
  .nav-link:hover{background:rgba(167,139,250,0.2);border-color:var(--purple);transform:translateY(-1px)}
  .nav-dot{font-size:.6rem}.nav-dot.online{color:var(--green)}.nav-dot.offline{color:var(--red)}
...
<div class="dash">
  ...
  <div class="row">
    <div class="stat-grid" style="grid-template-columns:repeat(4,1fr)">
      ...
      <div class="stat-card glow-amber"><span class="stat-val">${summary.totals.suggestions + summary.totals.vulnerabilities}</span><span class="stat-label">Findings</span></div>
  ...
  <!-- ── Mission Control: compact launcher ── -->
  <div class="row"><div class="panel highlight launchpad-panel" id="launchpad"><h2>Mission Control</h2>
    <input id="lp-area" type="hidden" value="${esc(selectedModule.area)}">
    <div class="mod-grid mod-grid-launch" style="margin-bottom:14px">${moduleCards}</div>
    <div class="launch-form">
      <div class="launch-row"><label>Goal</label><input id="lp-goal" type="text" placeholder="e.g. Audit auth middleware for gaps"/></div>
      ...
      <div class="launch-presets">
        <span class="launch-presets-label">Examples</span>
        <div class="preset-row">${presetButtons}</div>
      </div>
      <div class="launch-row launch-row-actions">
        <button id="lp-launch" onclick="launchMission()">LAUNCH MISSION</button>
        <span id="lp-status" class="launch-status" id="launch-guide"></span>
      </div>
    </div>
  ...
  <!-- ── Latest Activity: what ACTUALLY happened ── -->
  <div class="row"><div class="panel" style="border-color:var(--cyan)"><h2>Latest Activity</h2>
    ${latestAgent ? `
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;flex-wrap:wrap">
        <span class="status-badge s-${esc((latestAgent.status || "").replace(/_/g, "-"))}">${esc(latestAgent.status)}</span>
        <span style="font-family:var(--mono);font-size:.78rem;color:var(--text-dim)">${esc(latestAgent.track)} \u00b7 ${latestAgent.agent_goal_mode === "audit" ? "AUDIT" : "EDIT"}</span>
        <code style="font-size:.72rem">${esc(latestAgent.agent_model || "heuristic")}</code>
      </div>
      <p class="agent-summary">${esc(latestAgent.agent_summary || "No summary recorded.")}</p>

      ${(latestAgent.agent_applied_files || []).length ? `
        <div style="margin-top:16px;padding:16px;background:rgba(52,211,153,0.08);border:1px solid rgba(52,211,153,0.2);border-radius:10px">
          <h3 style="font-family:var(--mono);font-size:.82rem;color:var(--green);margin-bottom:10px">\u270f\ufe0f FILES EDITED (${latestAgent.agent_applied_files.length})</h3>
          <div class="detail-files" style="margin-bottom:8px">${latestAgent.agent_applied_files.map((f) => `<code class="applied-file" style="font-size:.8rem;padding:5px 10px">${esc(typeof f === "string" ? f : f.path || String(f))}</code>`).join(" ")}</div>
      ...
      ${(latestAgent.agent_selected_files || []).length ? `
        <div style="margin-top:14px;padding:14px;background:var(--surface2);border:1px solid var(--border);border-radius:10px">
          <h3 style="font-family:var(--mono);font-size:.82rem;color:var(--cyan);margin-bottom:10px">\ud83d\udd0d FILES INSPECTED (${latestAgent.agent_selected_files.length})</h3>
          <div class="detail-files">${latestAgent.agent_selected_files.map((f) => {
            const fp = typeof f === "string" ? f : f.path || f.relative_path || String(f);
            return `<code style="font-size:.76rem;padding:4px 8px">${esc(fp)}</code>`;
          }).join(" ")}</div>
        </div>
      ` : ""}

      ${(latestAgent.suggestion_findings || []).concat(latestAgent.vulnerability_findings || []).length ? `
        <div style="margin-top:14px">
          <h3 style="font-family:var(--mono);font-size:.82rem;color:var(--amber);margin-bottom:10px">\u26a0 FINDINGS (${(latestAgent.suggestion_findings || []).length + (latestAgent.vulnerability_findings || []).length})</h3>
          ${(latestAgent.suggestion_findings || []).concat(latestAgent.vulnerability_findings || []).map((f) =>
            `<div class="finding-card" style="margin:6px 0"><div class="finding-header"><span class="sev sev-${esc(f.severity || "info")}">${esc(f.severity || "info")}</span><span class="finding-title">${esc(f.title)}</span></div><div class="finding-detail">${esc(f.detail || "")}</div></div>`
          ).join("")}
        </div>
      ` : `<p style="color:var(--text-dim);font-size:.82rem;margin-top:12px">No findings from this mission.</p>`}

      ${latestAgent.agent_goal_mode === "audit" && !(latestAgent.agent_applied_files || []).length ? `
        <p style="color:var(--text-dim);font-size:.82rem;margin-top:14px;padding:12px;background:rgba(245,158,11,0.08);border:1px solid rgba(245,158,11,0.15);border-radius:8px">\ud83d\udca1 This was an <b>audit</b> (read-only). To have the agent edit files, launch with <b>Edit</b> mode.</p>
      ` : ""}
    ` : `<p class="empty">No missions run yet. Launch one above!</p>`}
  </div></div>

  <!-- ── All Findings: actionable ── -->
  <div class="row"><details class="panel" open><summary class="section-toggle"><h2>All Findings <button class="model-btn" onclick="event.stopPropagation();loadFindings()" style="font-size:.7rem;padding:3px 10px;margin-left:12px;vertical-align:middle" id="findings-refresh-btn">\u21bb load</button></h2></summary>
    <div id="findings-container"><p class="empty">Click "load" to fetch findings from all runs.</p></div>
  </details></div>

  <!-- ── Run History ── -->
  <div class="row"><details class="panel"><summary class="section-toggle"><h2>Run History (${summary.recent_runs.length})</h2></summary>
    <p class="table-hint">Click any row to expand details, inspected files, and findings.</p>
    <div style="overflow-x:auto"><table>
      <thead><tr><th></th><th>Status</th><th>Module</th><th>Run</th><th>Files Touched</th><th>Findings</th></tr></thead>
      <tbody>${recentRows || `<tr><td colspan="6" class="empty">No runs yet.</td></tr>`}</tbody>
    </table></div>
  </details></div>

  <!-- ── Model Roster ── -->
  <div class="row"><details class="panel"><summary class="section-toggle"><h2>Models <button class="model-btn" onclick="event.stopPropagation();refreshModels()" style="font-size:.7rem;padding:3px 10px;margin-left:12px;vertical-align:middle" id="model-refresh-btn">\u21bb refresh</button></h2></summary>
    <p style="color:var(--text-dim);font-size:.82rem;margin-bottom:12px">Active: <code>${esc(summary.runtime.model)}</code></p>
...
<div class="footer">KAAYKO API \u00b7 <button class="model-btn" onclick="manualRefresh()" style="font-size:.72rem;padding:4px 12px">\u21bb Refresh</button></div>
...
const BASE=(location.protocol==='file:'||location.origin==='null')?'http://localhost:7799':location.origin;
const SIBLING_BASE='http://localhost:4400';
let serverOnline=false;let activePollToken=null;

function esc(s){const d=document.createElement('div');d.textContent=s;return d.innerHTML}

async function checkSibling(){try{const r=await fetch(SIBLING_BASE+'/api/health',{signal:AbortSignal.timeout(2000),mode:'cors'});const dot=document.getElementById('frontend-dot');if(dot)dot.className='nav-dot '+(r.ok?'online':'offline')}catch{const dot=document.getElementById('frontend-dot');if(dot)dot.className='nav-dot offline'}}
...
function syncLaunchGuide(){const guide=document.getElementById('lp-status');if(!guide)return;if(!guide.textContent||guide.textContent.includes('disabled')||guide.textContent.includes('Choose')){if(serverOnline){guide.textContent='Ready to launch.';guide.style.color='var(--green)'}else{guide.textContent='Start kaayko-api serve first.';guide.style.color='var(--amber)'}}}

async function checkServer(){try{const c=new AbortController();const t=setTimeout(()=>c.abort(),2000);const r=await fetch(BASE+'/api/health',{signal:c.signal,mode:'cors'});clearTimeout(t);const h=await r.json();serverOnline=h.ok;if(h.busy&&h.activeMission){const el=document.getElementById('server-label');if(el)el.textContent='busy: '+h.activeMission.area+' ('+h.activeMission.elapsed+'s)';const d=document.getElementById('server-dot');if(d){d.className='server-dot online';d.style.background='var(--amber)';d.style.boxShadow='0 0 6px var(--amber)'}return}else{const d=document.getElementById('server-dot');if(d){d.style.background='';d.style.boxShadow=''}}}catch{serverOnline=false}const d=document.getElementById('server-dot'),l=document.getElementById('server-label');if(d){d.className='server-dot '+(serverOnline?'online':'offline');l.textContent=serverOnline?'serve active':'serve offline'}const b=document.getElementById('lp-launch');if(b)b.disabled=!serverOnline;syncLaunchGuide()}

async function launchMission(){if(!serverOnline)return;const a=document.getElementById('lp-area').value,g=document.getElementById('lp-goal').value.trim(),m=document.getElementById('lp-mode').value,s=document.getElementById('lp-status');if(!g){s.textContent='Goal is required';s.style.color='var(--red)';return}const b=document.getElementById('lp-launch');b.disabled=true;s.textContent='Launching...';s.style.color='var(--amber)';try{const r=await fetch(BASE+'/api/launch',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({area:a,goal:g,mode:m,goalMode:m==='dry-run'?'edit':m})});const d=await r.json();if(d.ok){s.textContent='Running (PID '+d.pid+')';s.style.color='var(--green)';activePollToken=d.logFile;document.getElementById('lp-log-area').style.display='block';pollLog()}else{s.textContent=d.error||'Failed';s.style.color=r.status===409?'var(--amber)':'var(--red)';b.disabled=false}}catch(e){s.textContent=e.message;s.style.color='var(--red)';b.disabled=false}}
