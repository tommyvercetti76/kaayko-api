...
const { validateCoordinates } = require('../utils/sharedWeatherUtils');
...
class UnifiedWeatherService {
    ...
    async getWeatherData(location, options = {}) {
        ...
        return this._addMetadata(standardizedData, startTime, false);
    }
    ...
    async getWeatherDataBatch(locations, options = {}) {
        ...
        return {
            success: true,
            results,
            summary: { total: locations.length, successful, failed }
        };
    }
    ...
    _addMetadata(data, startTime, cached) {
        return {
            ...data,
            metadata: {
                ...data.metadata,
