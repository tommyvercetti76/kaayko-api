...
async function getRateLimitStatus(key, type, maxRequests, windowSeconds = 60) {
  try {
    ...
    const rateLimitDoc = await db.collection('rate_limits')
      .doc(`${type}_${key}_${currentBucket}`)
      .get();

    const count = rateLimitDoc.exists ? (rateLimitDoc.data().count || 0) : 0;
    const remaining = Math.max(0, maxRequests - count);
    const resetAt = (currentBucket + 1) * windowSeconds * 1000;
...
/**
 * Cleanup expired rate limit documents (for scheduled job)
 * Deletes documents older than 2 hours
 * 
 * @returns {Promise<{deleted: number}>}
 */
async function cleanupExpiredRateLimits() {
  const expiryDate = new Date(Date.now() - (2 * 60 * 60 * 1000)); // 2 hours ago
  const expiryTimestamp = admin.firestore.Timestamp.fromDate(expiryDate);

  const expiredSnapshot = await db.collection('rate_limits')
  ...
  if (expiredSnapshot.size > 0) {
    await batch.commit();
    console.log(`[RateLimit] Cleaned up ${expiredSnapshot.size} expired documents`);
  }
