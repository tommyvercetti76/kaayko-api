...
async function getPaddleScoreForLocation(location) {
  try {
    ...
    // Apply model calibration for consistent ratings across all APIs
    const calibratedPrediction = calibrateModelPrediction(
      ...
      weatherData.forecast, // Pass forecast data for trend analysis
      { latitude: location.latitude, longitude: location.longitude }
    );

    // Use calibrated rating for consistency with heatmap and paddleScore APIs
    ...
    // Return paddle score data with calibration details (consistent with paddleScore API)
    return {
      rating: prediction.rating, // Calibrated rating in 0.5 increments
      ...
      // Calibration details for debugging
